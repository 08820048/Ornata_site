...
/**
 * PixelBlast 背景组件，使用 three 与 postprocessing 实现像素爆裂动画。
 * 该实现遵循 React Bits 官方提供的组件代码与用法。
 */
...
/**
 * 创建液体扭曲效果（postprocessing Effect）。
 */
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  const visibilityRef = useRef<{ visible: boolean }>({ visible: true });
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      if (noiseAmount > 0) {
        ...
        if (composer && composer.passes.length > 0) composer.passes.forEach((pass) => (pass.renderToScreen = false));
      ...
      const onPointerDown = (e: PointerEvent) => {
        ...
        const ix = (threeRef.current?.clickIx ?? 0) as number;
        (uniforms.uClickPos.value[ix] as THREE.Vector2).set(fx, fy);
      ...
      const animate = () => {
        ...
        if (liquidEffect) (liquidEffect.uniforms.get('uTime') as THREE.Uniform).value = uniforms.uTime.value;
        if (composer) {
          if (touch) touch.update();
          composer.passes.forEach((pass) => {
            if (!(pass instanceof EffectPass)) return;
            const effectPass = pass as unknown as { effects: Effect[] };
            effectPass.effects.forEach((effect) => {
              const u = effect.uniforms?.get('uTime') as THREE.Uniform | undefined;
      ...
      if (t.liquidEffect) {
        const uFreq = t.liquidEffect.uniforms.get('uFreq') as THREE.Uniform | undefined;
        if (uFreq) uFreq.value = liquidWobbleSpeed;
        const uStrength = t.liquidEffect.uniforms.get('uStrength') as THREE.Uniform | undefined;
